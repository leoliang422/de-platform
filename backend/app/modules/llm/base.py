...
ANSWER_TEMPLATES: dict[str, str] = {
    "knowledge": "你是数据开发领域专家。请针对下面的知识点给出准确、简洁、有重点的中文 Markdown 参考答案："  # noqa: E501
    "先用一句话给出结论/定义，再用少量要点简要展开原理与关键点，避免冗长堆砌。\n" + _CLEAN_RULES,
    "sql": "你是数据开发面试官。请针对下面的 SQL 题目作答：先用一段话简述解题思路，"
    "再给出最终 SQL（放进带语言标注的代码块，关键处加注释），必要时补充一句注意事项。\n"  # noqa: E501
    + _CLEAN_RULES,
    "interview": "你是资深数据开发面试者。请用面试口头回答的风格，针对下面的面试问题简明扼要作答，"
    "直击要点、控制在约 150~300 字，不要长篇大论。\n" + _CLEAN_RULES,
    "project": "你是数据开发项目导师。请针对下面的项目相关问题给出专业、简洁的中文参考答案。\n"
    + _CLEAN_RULES,
}
