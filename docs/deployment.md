...
> 现状：站点**仅用积分解锁**，暂不接入现金/微信/支付宝（前端入口已移除，后端支付代码保留待用）。
>
> - **八股**：全部免费开放。
> - **项目**：管理员/投稿可把 `access_type` 设为「积分解锁」并填 `price_points`，按单个项目解锁（永久）。
> - **SQL / 面经**：每个模块每个用户**免费查看 N 条**，超出后**一次性用积分解锁整个模块**（永久可见）。
>
> 可调环境变量（均有默认值，不填即用默认）：
>
> | 变量 | 默认 | 含义 |
> | --- | --- | --- |
> | `FREE_MODULE_QUOTA` | `10` | 每个模块每用户的免费条数 |
> | `SQL_MODULE_UNLOCK_POINTS` | `50` | 一次性解锁 SQL 模块所需积分 |
> | `INTERVIEW_MODULE_UNLOCK_POINTS` | `50` | 一次性解锁面经模块所需积分 |

## 支付接入（微信 / 支付宝）

> 现状：**前端支付入口已下线，仅保留积分兑换**；以下为后端支付通道，配齐凭证后可随时重新启用。
> 代码已把**支付抽象层、异步下单、回调结算（webhook）**都搭好，微信/支付宝
> 走「占位」模式——**默认 `PAYMENT_PROVIDER=mock`，同步结算**。当你把
...
免费档最大的体验杀手是 **Render 免费实例 15 分钟无访问就休眠**，下一个访客要等 30–60s 冷启动。
在升级付费档之前，可用外部定时器「保活」把它顶住（不花钱）：

1. 打开免费定时服务，如 [cron-job.org](https://cron-job.org)（注册即可）。
2. 新建任务：URL 填后端健康检查 `https://<你的后端域名>/health`，方法 `GET`。
3. 频率设 **每 10 分钟**一次（比 15 分钟休眠阈值小即可）。
4. 保存启用。此后后端基本常驻，冷启动问题消失。

> 说明：这只是过渡方案；正式放量仍建议把 Render 后端升到 Starter（常驻、内存翻倍）。
> 前端已在 `lib/api.ts` 里对所有请求加了 60s（大模型解析 90s）超时，即使后端慢也不会让页面无限转圈。

其它已内置的零成本项：
- `render.yaml` 已把 `SEED_ON_START` 置 `0`（上线后不再每次部署重灌种子/覆盖运营数据）。
- 注册开启了邮箱验证码（`EMAIL_VERIFICATION_REQUIRED=true`）；未接真实 SMTP 时为 mock：
  验证码随接口返回并在前端自动填入，接入 SMTP 后自动变为真实校验（见下「邮件发送」）。
